...
FOLDER_PATH = "Weights/Random Offset/13/"
FOLDER_DATA_PATH = "Weights/Random Offset/13/"
...
def cutData(rawSamples, rawEvents, rawLabels, outputSignalLength):
    ...
    for sample, event, label in zip(rawSamples, rawEvents, rawLabels):
        ...
        glob_signal_events = list(zip(label, np.transpose(glob_signal_events)[0])) # cria tuplas com a coordenada do evento e o seu respectivo label (label, amostra de ocorrencia do evento)
        events_duration = []
        while len(glob_signal_events) != 0:
            for i in range(1, len(glob_signal_events)):
                if(glob_signal_events[0][0] == glob_signal_events[i][0]):
                    events_duration.append([glob_signal_events[0][0], glob_signal_events[0][1], glob_signal_events[i][1]])
                    del glob_signal_events[i]
                    del glob_signal_events[0]
                    break
        ...
        while (initIndex + outputSignalLength) < len(sample):
            ...
            if len(ev) > 0:
                for _ in range(AUGMENTATION_RATIO):
                    ...
                    randomizedInitCoordinates = initIndex + ev[0][0] - randrange(0, outputSignalLength) #+ ev[0][0] - (N_GRIDS - 1 - i) * gridLength - randrange(int(gridLength * MARGIN_RATIO), int(gridLength * (1 - MARGIN_RATIO)))
