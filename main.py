...
configs = {
    ...
    "TRAIN_SIZE": 0.8,
    "FOLDER_PATH": "80_nao_fez_diferenca/",
    "FOLDER_DATA_PATH": "80_nao_fez_diferenca/", 
...
def main():
    ...
    if EXECUTION_STATE == TRAIN:
        ...
        while fileEpoch < configs["TOTAL_MAX_EPOCHS"]:
            ...
            model.save(folderPath + 'single_loads_aug_multipleOutputs_' + str(signalBaseLength) + "_" + str(fileEpoch) + '.h5')
        ...
        postProcessing.checkModel(postProcessing.load_model(folderPath + 'single_loads_aug_multipleOutputs_12800_250.h5'), x_test, y_test)
        bestModel = modelHandler.loadModel(folderPath + "best_model.h5")
        postProcessing.checkModel(bestModel, x_test, y_test)
        MultiLabelMetrics.F1Macro(bestModel, x_test, y_test)
