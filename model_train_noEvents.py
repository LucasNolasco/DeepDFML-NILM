# Inclui o treino com fatias da amostra que não possuem eventos
...
def splitData(x, y, ratio):
    ...
    noEventsTrainLength = round(ratio * (len(x) - 32 * 26))
    generalTrainLength = trainLength
    for xi, yi in zip(x, y):
        if yi[1] == 1 or yi[2] == 1:
            generalTrainLength = trainLength
            lab = str(np.argmax(yi[3:]))
        else:
            generalTrainLength = noEventsTrainLength
            lab = "NoEvents"
        
        if lab not in dict_labels:
            dict_labels[lab] = 0

        if dict_labels[lab] < generalTrainLength:
            if not beginTrain:
...
def sumSquaredError(y_true, y_pred):
    weights = K.ones((2, 29))
    weights = weights[0][0].assign(5)
    weights = weights[1][0].assing(0)
    for i in range(3,29):
        weights = weights[1][i].assing(0)
    
    eq = K.equal(y_true[1], y_true[2])
    index = K.cast(eq, "int32")

    return K.sum(K.square(y_true - y_pred) * weights[index], axis=-1)
...
def main():
    ...
    while fileEpoch < 500:
        ...
        model.save('single_loads_noEvents_' + str(SIGNAL_LENGTH) + "_" + str(fileEpoch) + '.h5')
